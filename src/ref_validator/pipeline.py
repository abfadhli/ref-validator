...
class ValidationPipeline:
    ...
    async def _verify_one(
        ...
        async with self._semaphore:
            ...
            metadata_ok = metadata.title_match  # title match is the minimum
            if level == VerificationLevel.METADATA:
                status = VerificationStatus.VERIFIED if metadata_ok else VerificationStatus.PARTIAL
                progress.on_advance()
                return CitationVerification(
                    ref_id=reference.ref_id,
                    reference=reference,
            ...
            if not claim_results:
                ...
                all_supported = all(cr.supported is True for cr in claim_results)
                any_contradicted = any(cr.supported is False for cr in claim_results)
                if all_supported and metadata_ok:
                    status = VerificationStatus.VERIFIED
